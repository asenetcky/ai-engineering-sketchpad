...
def _(client):
    def get_patient_info(transcription):
        ...
        input=[
            {
                "role":"system", 
                "content":"You are a medical record expert. For each transcription return both the age of the patient and recommended treatment. If there is missing information, create the field and specify 'unknown'",
                "role":"user",
                "content": f"Please extract the information from the transcript between the backticks ```{transcription}```"
            }
        ]
        ...
        tools = [
            {
                ...
                'function': {
                    ...
                    'description': 'Get patient age and recommended treatment.',
                    'parameters': {
                        'type': 'object',
                        'properties': {
                            'age': {
...
@app.cell
def _(transcription_data):
    transcription = transcription_data.to_dicts()[0]["transcription"]
    transcription
    return (transcription,)


@app.cell
def _(client, transcription):
...
@app.cell
def _(get_patient_info, test_transcription):
    get_patient_info(test_transcription)
    return
...
def _():
    ...
    # for patient in transcription_data.to_dicts():
    #     # print(patient)
    #     transcription = patient["transcription"]
    #     # print(transcription)
    #     # details = []
    #     info = get_patient_info(transcription)
    #     print(info)
    #     # icd = get_icd_codes(info)

    #     # info.append(icd)
    #     # patient_details.append(into)
