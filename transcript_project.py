...
@app.cell
def _(client):
    def get_patient_info(transcription):
...
@app.cell
def _(get_transcript_info, json, transcription_data):
    test_data = transcription_data.to_dicts()[0]
    print(test_data)

    print(get_medical_speciality(test_data))

    _info = get_transcript_info(test_data)
    print(_info)

    info_dict = json.loads(_info)
...
@app.cell
def _(get_patient_info, transcription_data):
    # for testing
    for transcript_dict in transcription_data.to_dicts():
            transcript = transcript_dict["transcription"]
        # print(transcript)
            pt_info = get_patient_info(transcript)
        # print(pt_info)

            extract = pt_info.choices[0].message.tool_calls[0].function.arguments
...
@app.cell
def _(get_patient_info, json):
    def get_transcript_info(transcript)-> dict:
        pt_info = get_patient_info(transcript)
        info = pt_info.choices[0].message.tool_calls[0].function.arguments

        info = json.loads(info)

        return(info)

    return (get_transcript_info,)
        ...
        if treatment == "unknown":
            ...
            response = client.chat.completions.create(
               ...
               messages=[
                   {
                       "role":"user",
                       "content":f"Provide the ICD10 codes from the treatment between the backticks ```{treatment}```. Return the answer as a list of codes and no other information"
                   }
               ],
           )
...
def _(get_icd_codes, get_transcript_info):
    def compile_dict(transcription):
        ...
        for transcript_dict in transcription.to_dicts():
            ...
            medical_speciality = get_medical_speciality(transcript_dict)
            icd = get_icd_codes(info.get("procedure"))

            dict["age"] = info["age"]
            dict["procedure"] = info["procedure"]
            dict["speciality"] = medical_speciality
            dict["icd"] = icd
            # print(info)
            # print(medical_speciality)z
            # print(icd)
...
@app.cell
def _(data, pl):
    df_structured = pl.DataFrame(data).to_pandas()
    df_structured
    return
